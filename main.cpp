...
void showEngineer(){
    ...
    for (int i = 0; i < cash.getSize(); i++) {
        cout << i <<  ". " << cash.at(i)->name << " " << cash.at(i)->surname << endl;
    }
    cout << endl;
}
        ...
        switch (utility::menuWithArray(strdup("Выберете режим работы:"),menu3,6)) {
            case 0:
                if(cash.at(index)->is_work()){
                    ...
                    cash.at(index)->work(task);
                }
                break;
            case 1:
                cash.at(index)->endWork();
                cout << line << "Инженер завершил работу." << line;
                break;
            case 2:
                cout << line << "Количество сделанных заданий: "<<  cash.at(index)->completed_tasks << line;
                break;
            case 3:
                if(cash.at(index)->is_work()){
                    cout << line;
                    cout << "Инженер " << cash.at(index)->name  << " " << cash.at(index)->surname << " работает над: " << cash.at(index)->task_name;
                    cout << line;
                } else{
                    cout << line;
                    cout << "Инженер " << cash.at(index)->name  << " " << cash.at(index)->surname << " свободен.";
                    cout << line;
                }
                break;
            case 4:
...
int select(){
    ...
    do{
        do{
            ...
            cout << cash.getSize() + 1 << " -> " << "Назад " << endl;
            cin >> selected;
        } while (selected < 0 or selected > cash.getSize() + 1);
        if(selected == cash.getSize() + 1){
            return -1;
        }
        return selected;
    } while (true);
...
void addEngineer(){
    ...
    cout << "Фамилия: ";
    cin >> surname;
    cash.push_back(new Engineer(name,surname));
...
void load_file(){
    ...
    for(int i = 0;i<rr;i++){
        ...
        while (ss <= 5);
        cash.push_back(new Engineer(name,surname,task_name,com_task,work));
    }
    f.close();
...
void save_file(){
    ...
    if (f.is_open())
    {
        for(int i = 0;i<cash.getSize();i++){
            ...
            f << cash.at(i)->name << endl
              << cash.at(i)->surname << endl
              << cash.at(i)->task_name << endl
              << cash.at(i)->completed_tasks << endl
              << (int)cash.at(i)->is_work();
        }
    }
    f.close();
